...
var generatedConfigs_default = configs;
...
const noTopLevelFunctionExpression = {
  create(context) {
    ...
    const scopeTypes = /* @__PURE__ */ new Set(["global", "module"]);
    const hasGlobalReturn = context.languageOptions.parserOptions?.ecmaFeatures?.globalReturn === true;
    const messageId = "noTopLevelExpression";
    return {
      VariableDeclarator: (node) => {
        const nodeType = node.init?.type;
        if (nodeType === void 0 || !nodeTypes.has(nodeType)) {
          return;
        }
        const scope = context.sourceCode.getScope(node);
        const currentScope = hasGlobalReturn ? scope.upper : scope;
        const isTopLevel = currentScope != null && scopeTypes.has(currentScope.type);
        if (!isTopLevel) {
          return;
        }
        context.report({
  ...
  meta: {
    messages: {
      noTopLevelExpression: "Top-level function expressions are not allowed. Use function declarations instead."
    },
    type: "suggestion"
  }
};
var no_toplevel_function_expression_default = noTopLevelFunctionExpression;

const kylsgl = {
  rules: {
    "no-toplevel-function-expression": no_toplevel_function_expression_default
  }
};
var kylsgl_default = kylsgl;

const plugins = {
  base: {
    "@stylistic": stylistic,
    "import-x": importX,
    kylsgl: kylsgl_default,
    n,
    perfectionist,
    promise,
    regexp,
...
var ignores_default = [ignoresConfig];

const config = [
  {
    languageOptions: {
      ecmaVersion: "latest",
      parser: parsers_default.typescript,
      parserOptions: {
        ecmaVersion: "latest",
        projectService: true,
    ...
    settings: {
      ...settings_default.base,
      ...settings_default.typescript
    }
  },
  ...generatedConfigs_default.base,
  ...generatedConfigs_default.typescriptTypeChecked,
  ...generatedConfigs_default.prettier,
  ...ignores_default
];
